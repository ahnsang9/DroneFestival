...
class Ui_Dialog(object):
    def __init__(self, Dialog, device, bObjectDetection):
        ...
        ########################################################################################################3
        self.sheperd_Mode = 0  # Toggle을 위한 옵션----------------------------------------------------------------------------
        ########################################################################################################3
        ...
        try:
            if self.tello.cap is not None:
                ...
                if self.originFrame is not None:
                    ...
                    if self.bObjectDetection:
                        ...
                        if detectedObjects is not None:
                            for object in detectedObjects:
                                ...
                                if class_id == 15:
                                    self.sheep_pos = [xmax, ymax, xmin, ymin]
                                    self.exi_sheep = 1
                                elif class_id == 7:
                                    self.door_pos = [xmax, ymax, xmin, ymin]
                                    self.exi_door = 1
                                else:
                                    self.sheep_pos = []
                                    self.exi_sheep = 0
                                ...
                                for i in range(6):
                                    if self.latestDetectedLabel != self.picLabels[i] and self.objDetector.labels[
                                        class_id] == self.picLabels[i]:
                                ...
                                cv2.rectangle(renderImg, (xmin, ymin),
                                              (xmax, ymax), (0, 0, 0), 2)
                                cv2.putText(renderImg, self.objDetector.labels[class_id], (xmin, ymin),
                        ...
                        for i in range(6):
                            if self.labelDetectCounts[i] > self.DETECTION_FRAME_COUNT:
                                ...
                                # 이미지 업데이트
                                onlyObjFrameBGR = self.originFrame[
                                                  self.labelDetectInfo[i][1]:self.labelDetectInfo[i][3],
                                                  self.labelDetectInfo[i][0]:self.labelDetectInfo[i][2]]
                    ...
                    qImg = QtGui.QImage(renderImg.data, renderingW, renderingH, bytesPerLine,
    ...
    def sheperd(self):
        ...
        if self.sheperd_Mode == 1:
            if self.sheep_pos != []:
                self.sheep = self.sheep_pos
                self.sheepx_pos = [int((self.sheep[0] + self.sheep[2]) / 2), 362]
                self.dif_sheep = self.originpoint[0] - self.sheepx_pos[0]
                self.dist_sheep = self.sheep[0] - self.sheep[2]

            if self.door_pos != []:
                self.door = self.door_pos
                self.doorx_pos = [int((self.door[0] + self.door[2]) / 2), 362]
                self.dif_door = self.originpoint[0] - self.doorx_pos[0]
                self.dist_door = self.door[0] - self.door[2]

            if self.exi_sheep == 0 and self.findex == 0:
                cmd = "cw %d" % self.delta_rotate
                self.log(cmd)
                self.tello.send_command(cmd)

                if self.sheeptime == self.sheeptime + 10:
                    self.drone.land()
                    self.findex = 1
                    self.sheperd_Mode = 0
                    print("더 이상 탐지되는 양이 없습니다.")

    def sheperdtrue(self):
        if self.sheperd_Mode == 1:
            if self.exi_sheep == 1:
                if -20 >= self.dif_sheep:
                    cmd = "cw %d" % self.delta_rotate
                    self.log(cmd)
                    self.tello.send_command(cmd)

                elif 20 <= self.dif_sheep:
                    self.drone.counterclockwise(45)

                elif -20 <= self.dif_sheep <= 20:
                    if self.dist_sheep <= 300:
                        self.drone.forward(30)

                    elif self.dist_sheep >= 300:
                        if self.exi_door == 0:
                            cmd = "curve 70 70 0 140 0 0 20"
                            self.log(cmd)
                            self.tello.send_command(cmd)
                            cmd = "cw %d" % self.delta_rotate
                            self.log(cmd)
                        ...
                        if self.dist_door <= 600:
                            cmd = "forward %d" % self.delta_FB
                            self.log(cmd)
                            self.tello.send_command(cmd)

                        elif self.dist_door >= 600:
                            cmd = "land"
                            self.log(cmd)
                            self.tello.send_command(cmd)
    ...
    def rotateCW(self):
        if self.tello.socket is None:
            self.log("tello is not connected.")
            return
        self.sheperd_Mode = not self.sheperd_Mode
        self.sheeptime = time.time()
        while self.sheperd_Mode == 1:
            self.sheperd()

        cmd = "Sheperd Mode started"
        self.log(cmd)
